function NGFac(num) //普通生成器效果
{
    if( num <= 1 )
    {
        ...
        if( greater( game.infinity.number[4] , new bigNum(0,0) ) )
        {
            var nanFac = mul( add( pow( game.infinity.energy , 1 / 10 ) , new bigNum(1,0) ) , mul( game.normal.speedUpFac , add( numToExp( game.infinity.times ) , new bigNum(1,0) ) ) );  //一巨坨公式
        }
        else
        {
